...
def query_llava(image_path, prompt, model="llava", ollama_url="http://localhost:11434/api/generate", timeout=60):
    """
    Query LLaVA via Ollama API with debug output and timeout.
    """
    print(f"[LLaVA] Sending prompt to Ollama: {prompt}")
    ...
    try:
        ...
        print(f"[LLaVA] POST {ollama_url} ...")
        response = requests.post(ollama_url, json=payload, timeout=timeout)
        response.raise_for_status()
        print(f"[LLaVA] Got response: {response.status_code}")
        return response.json()["response"]
    except Exception as e:
        print(f"[LLaVA] ERROR during API call: {e}")
        return ""
